...
export const reactIntroArticles: ReactIntroArticle[] = [
  {
    ...
    sections: [
      {
        title: "まず、Reactは何をしてくれるのか",
        body: [
          "Reactは、Web画面を作るためのライブラリです。ここでいう画面とは、ただの見た目だけではありません。ボタンを押したら数字が増える、入力した名前がプレビューに出る、ログインしている時だけメニューが出る。そういう「変化する画面」を作るための道具です。",
          "昔ながらの書き方では、画面のどこを探して、どの文字を書き換えて、どの要素を消して、どの要素を追加するかを細かく命令します。小さいうちはそれでも動きます。けれど画面が大きくなると、どこを直せばいいのか分かりにくくなります。",
          "Reactでは、画面を小さな部品に分けます。ボタン、カード、入力欄、一覧、ヘッダー。こうした部品をコンポーネントとして作り、組み合わせて画面を作ります。部品に名前が付くので、コードを読むときにも「ここは記事カードだな」「ここは検索フォームだな」と見通しがよくなります。",
        ],
      },
      {
        title: "Reactを使わない場合、何が大変なのか",
        body: [
          "たとえば「わかった」ボタンを押すたびに数字が増える画面を考えます。Reactを使わずに書くなら、ボタンを探し、数字を表示している場所を探し、現在の数字を読み取り、1を足して、文字を書き換える必要があります。",
          "この処理が1か所ならまだ平気です。でも、数字をヘッダーにも表示したい、カードにも表示したい、保存ボタンの有効/無効にも使いたい、となると一気に散らかります。同じ値を使う場所が増えるほど、「どこを書き換えるんだっけ？」が増えます。",
          "Reactの考え方は逆です。画面を直接探して書き換えるのではなく、元になる値を変えます。そして、その値を使っている画面をReactがもう一度作ります。だから、考える中心が「画面の場所」ではなく「値」になります。",
        ],
        code: {
          filename: "Reactを使わない時に考えがちなこと",
          value: `
ボタンが押された
  ↓
数字を表示している場所を探す
...
画面の文字を書き換える
      ...
      {
        title: "Reactでは、値から画面を作る",
        body: [
          "Reactでは、値が先にあります。countが0なら「0回」と表示し、countが1なら「1回」と表示します。画面は、今の値から作られた結果です。",
          "この見方に慣れると、画面づくりが少し楽になります。どこを直接書き換えるかではなく、「どの値が変わるのか」「その値をどこで表示しているのか」を見ればよくなるからです。",
          "父が娘に説明するなら、Reactは「画面の自動更新係」です。こちらが値を変えると、Reactがその値を見て、必要な表示に作り直してくれます。",
        ],
        code: {
...
画面が新しい表示になる
      ...
      {
        title: "なぜ多くの現場で選ばれるのか",
        body: [
          "Reactが選ばれる理由は、書き方がかっこいいからではありません。大きくなっていく画面を、部品と値の変化として整理しやすいからです。",
          "また、使っている人が多いことも大きな理由です。分からないことを検索しやすい。UI部品やフォーム、テスト、Next.jsのような周辺技術も充実している。学び始めた人にとっても、仕事で使う人にとっても、情報が多いのはかなり助かります。",
          "もちろんReactだけで全てが解決するわけではありません。ルーティング、サーバー処理、データベース、認証、デプロイなどは別の知識も必要です。だからこの連載では、まずReactの見方を固め、そのあとTypeScriptやNext.jsへ進みます。",
        ],
      },
    ],
  },
  {
    ...
    sections: [
      {
        title: "Reactでは、画面は結果である",
        body: [
          "Reactを理解するときに一番大事なのは、「画面は結果である」という見方です。画面を直接いじるのではなく、元になる値を変えます。するとReactが、その値から新しい画面を作ります。",
          "たとえばcountという値が0なら、画面には「0回」と表示されます。countが1なら「1回」です。このとき、画面の文字を直接探して書き換えているのではありません。countが変わったので、Reactがコンポーネントをもう一度実行し、新しいJSXを作っています。",
          "この考え方は最初少し遠回りに見えます。でも、値と表示の関係がはっきりすると、画面が複雑になっても追いやすくなります。",
        ],
      },
      {
        title: "小さなカウンターで見る",
        body: [
          "ここでは、カウンターを例にします。ボタンを押すとcountが増えます。countが増えると、ReactはCounterコンポーネントをもう一度計算します。そして、{count}を使っている場所に新しい値が表示されます。",
          "大事なのは、setCountを呼んだ瞬間に「Reactに値が変わった」と伝わることです。Reactはその合図を受けて、次の画面を作ります。",
          "父の言葉で言うと、setCountは「画面更新の呼び鈴」です。直接countを書き換えるのではなく、呼び鈴を鳴らしてReactに知らせます。",
        ],
        code: {
  ...
  return (
    <button onClick={() => setCount(count + 1)}>
      ...
      },
      {
        title: "再描画は怖くない",
        body: [
          "再描画とは、コンポーネントをもう一度呼び直して、新しい画面を作ることです。言葉だけ聞くと、画面全部を毎回作り直していて重そうに感じます。",
          "でも、まず入門段階では怖がらなくて大丈夫です。Reactは前の結果と新しい結果を見比べ、実際の画面に必要な変更を反映します。私たちはまず、値が変わるとコンポーネントがもう一度計算される、というモデルを持てば十分です。",
          "最適化は後で学べます。最初からmemoやuseMemoのような話に進むと、肝心の見方がぼやけます。まずは、値、コンポーネント、表示結果の関係を体に入れます。",
        ],
      },
      {
        title: "propsとstateの違いも、この見方で分かる",
        body: [
          "propsは外から渡される値です。親コンポーネントから子コンポーネントへ流れてきます。stateはコンポーネント自身が覚えている値です。",
          "どちらも、画面を作る材料です。propsが変わっても、stateが変わっても、その値を使うコンポーネントは新しい表示を作ります。",
          "つまりReactの頭の中では、「材料が変わったから、料理を作り直そう」ということが起きています。料理のたとえを出した父は、このあと夕飯の献立で現実に戻ります。",
        ],
        code: {
          filename: "propsとstateの役割",
          value: `
props: 外から渡される材料
state: 部品自身が覚えている材料
...
画面が変わる
  ...
  },
  {
    ...
    sections: [
      {
        title: "JSXはHTMLに似ているけれど、HTMLそのものではない",
        body: [
          "JSXを見ると、ほとんどHTMLに見えます。h1、p、button、section。見慣れたタグが並ぶので、最初は「HTMLをJavaScriptの中に書いているのかな」と感じるかもしれません。",
          "近いですが、まったく同じではありません。JSXは、Reactが画面を作るために読む設計図です。JavaScriptの中に書けるので、変数や関数の結果を画面に差し込めます。",
          "この「JavaScriptの値を画面に混ぜられる」ことがJSXの大事なところです。固定のHTMLを書くのではなく、今の値に合わせた画面を書きます。",
        ],
      },
      {
        title: "波かっこで値を画面に出す",
        body: [
          "JSXの中でJavaScriptの値を使うときは、波かっこを使います。たとえば{name}と書くと、nameという変数の中身が画面に表示されます。",
          "これはReactの入門で何度も出てくる形です。propsの値を表示するときも、stateの値を表示するときも、配列をmapで表示するときも、波かっこが出てきます。",
          "波かっこは、父が娘に説明するなら「ここだけJavaScriptを入れていいよ、という窓」です。",
        ],
        code: {
...
export function Greeting() {
  return <p>こんにちは、{name}さん。</p>;
      ...
      {
        title: "JSXでは式は書けるが、文はそのまま書けない",
        body: [
          "JSXの波かっこの中には、JavaScriptの式を書けます。式とは、最終的に値になるものです。name、count + 1、isLogin ? 'ようこそ' : 'ログインしてください' などです。",
          "一方で、if文やfor文をそのままJSXの中に置くことはできません。条件分岐をしたいなら、returnの前でifを書くか、JSXの中では三項演算子を使います。",
          "この違いは少しややこしいですが、まずは「波かっこの中には値になるものを書く」と覚えれば十分です。",
        ],
        code: {
          filename: "JSXで値を差し込む",
          value: `
...
export function Status() {
  return <p>{message}: {count}個</p>;
}
          `,
        },
      },
      {
        title: "classではなくclassName、forではなくhtmlFor",
        body: [
          "JSXでは、HTMLのclass属性をclassNameと書きます。JavaScriptのclassという言葉とぶつからないようにするためです。",
          "labelタグのfor属性も、JSXではhtmlForと書きます。これもJavaScriptの予約語とぶつからないようにするためです。",
          "細かい違いはいくつかありますが、最初は「HTMLに似ているけれど、JavaScriptの中にいる」と覚えると十分です。エラーが出たら、HTMLとの差分をひとつずつ覚えれば大丈夫です。",
        ],
        code: {
          filename: "JSXでの属性名",
          value: `
export function NameInput() {
  return (
    <label htmlFor="name" className="label">
      ...
      {
        title: "JSXは見た目とロジックの距離を近づける",
        body: [
          "JSXに慣れると、表示と値の関係が近くに見えるようになります。nameを表示している場所、countを表示している場所、条件で切り替えている場所が、コード上で見つけやすくなります。",
          "これはReactの読みやすさにつながります。HTML、CSS、JavaScriptが完全に別々の場所に散らばるのではなく、ひとつのコンポーネントの中で「この部品は何を表示するのか」をまとめて読めます。",
          "もちろん、大きくなったら分け方も考えます。でも最初は、JSXによって画面と値の関係が近くなることを感じるのが大事です。",
        ],
      },
    ],
  },
